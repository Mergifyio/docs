...
- `src/content/enterpriseReleaseNotes/` - Per-release upgrade notes for on-premise
...
6. **Never modify changelog entries** - Files in `src/content/changelog/`
   ...
   The same goes for `src/data/enterprise-releases.json`, the list of
   on-premise releases, which a bot syncs when a release is cut.
...
8. **Never publish internal information** - Ticket IDs, customer names, private
   ...
   `src/content/docs/` or `src/content/enterpriseReleaseNotes/`. See the section
   below; it applies to every change, with no size threshold.

## Enterprise Release Notes

`/enterprise/releases` lists every on-premise release and what it asks of whoever
performs the upgrade. It joins three sources and only one of them is written by
hand:

- `src/data/enterprise-releases.json` - the tags, synced by a bot. Never edited here.
- the `changelog` collection - windowed by release date to derive the changes a
  release carries. Never edited here either (Critical Rule 6).
- `src/content/enterpriseReleaseNotes/` - one optional file per release, holding
  what an operator has to do.

So a release reaches the page with no work at all, and a file is added only when
a release renames an environment variable, removes a configuration option, needs
a migration step, or changes a default in a way that alters load. The README in
that directory is the full procedure; `src/util/enterpriseReleases.ts` documents
why the changelog window is derived rather than recorded.

These notes are written *from* internal release material - upgrade notes,
configuration diffs, tag ranges - so they carry the same leak risk as any docs
page, and `pnpm check:internal-leaks` scans them alongside `src/content/docs/`.
