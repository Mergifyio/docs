...
You can use regular expression with matching operators in your <Link
to="../conditions">conditions</Link>.
...
<Alert>
   ...
   [PyRegex](http://www.pyregex.com) [Pythex](https://pythex.org) to test your
   regular expressions.
</Alert>
...
However, those templates allow to use any of the <Link
to="../conditions/#attributes-11">pull request attribute</Link> in the final
string.
...
<Alert>
   You need to replace the `-` character by `_` from the <Link
   to="../conditions#attributes-1">pull request attribute </Link> names when
