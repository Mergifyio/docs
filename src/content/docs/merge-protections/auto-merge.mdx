...
Only matching pull requests are auto-merged. Other pull requests still need to
be merged manually or queued via the [`@mergifyio queue` command](/commands/queue).
...
| omitted | — | No automatic action. Merge manually or use the [`@mergifyio queue` command](/commands/queue) |
...
The schema rejects `false` and any mapping, empty (`{}`) or not. An explicit
`null` is accepted and means the same as omitting the field: auto-merge stays
off.
...
Those queues stay reachable via the [`@mergifyio queue` command](/commands/queue).

Apply the migration in one step: a configuration that sets
`auto_merge_conditions` while a queue rule still has `autoqueue` is rejected.
...
`auto_merge_conditions` and `queue_rules[].autoqueue` cannot be used together.
A configuration that sets both is rejected, so remove `autoqueue` from your
queue rules in the same change.

Without the merge queue, `auto_merge_conditions` only takes effect when at
least one [merge protection rule](/merge-protections/custom-rules) applies to
the pull request: with no rule whose `if` conditions match, there are no
`success_conditions` to evaluate and nothing is merged. With the merge queue
enabled, a matching pull request is queued whether or not merge protection
rules exist, and the queue's own conditions gate the merge.
