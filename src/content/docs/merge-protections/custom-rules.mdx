...
Rules live under the `merge_protections` key in your configuration file. Each
rule needs `name`, `if`, and `success_conditions`; `description` is optional:
...
Each example below is one item of the `merge_protections` list.
