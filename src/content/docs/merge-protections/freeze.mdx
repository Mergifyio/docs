...
2. Fill in the reason at the top of the editor. It is required, and it names
   the freeze wherever Mergify reports it

3. Under `Schedule`, set `Starts` and `Ends`, and the `Timezone` they are read
   in. Leave `Ends` empty for a freeze that lasts until you remove it

4. Under `Scope`, add the [conditions](/configuration/conditions) a pull
   request must match to be blocked. Leave it empty to freeze every pull
   request on the repository

5. Under `Exceptions`, add the conditions that let a pull request through the
   freeze, so specific pull requests (like hotfixes) can still merge while
   everything else is blocked

6. Click `Save Freeze`; the freeze appears in the list under `Upcoming` until
   it starts
...
:::caution
  Mergify evaluates the freeze, but only GitHub can stop a merge made outside
  Mergify. Unless your branch requires `Mergify Merge Protections` to succeed
  (as a check, or as a deployment if you set `reporting_method: deployments`),
  someone can still merge a pull request straight from the GitHub UI while the
  freeze is active. See [Make the Check
  Required](/merge-protections/setup#make-the-check-required) to close that
  gap.
:::

## Manual Instant Freeze

Use the `Freeze Now` action for immediate blocking. It opens the same editor
without a start date: pull requests matching `Scope` are blocked the moment you
save. Set an end date and the freeze lifts itself, or leave it empty and remove
the freeze when you are done.
...
Set exclude conditions when you create or edit a freeze, in the dashboard's
`Exceptions` section or from the CLI with the `-e` flag (see [Creating a
