...
A pull request that deletes the configuration file rather than editing it never
gets a `Configuration changed` check. It reports under `Configuration has been
deleted` instead, which Mergify posts as a success, and that check satisfies
this protection too. Removing the configuration is an accepted outcome, not a
failure.
