...
Queues are the one part of this page you have to configure: they are grouped by
the labels you record, and a job that requests only unrecorded labels is not
counted. See [Recording the labels you care about](#recording-the-labels-you-care-about).
...
- **A runner shows up once it's reused.** Mergify holds off the first time a runner
  ...
  persistent machine and starts tracking it. Only first attempts count: a job that
  a runner picks up as a re-run does not push it over the line. This keeps your
  dashboards free of the ephemeral runner identities that large fleets churn
  through.
...
## Recording the labels you care about

Runners need no configuration, but the labels do. Under **Self-hosted runners →
Settings**, the **Runner Labels** field lists the labels Mergify records; it
suggests the ones it has seen in your CI events, and you add or remove them
from there.

Queue metrics are grouped by these labels, and a job that requests none of the
recorded labels is not counted as demand at all. Until you record at least one
label, a fleet whose jobs all specify `runs-on` labels produces no queue data.
The Fleet view is unaffected: runners are tracked either way, and recording
labels only adds the per-label breakdown.
