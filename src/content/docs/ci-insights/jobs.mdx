...
:::tip
  Group the metrics by branch to compare the same job on your default
  branch and on pull requests. Filters on job name, pipeline name, branch,
  and health narrow the list further. The repository selector at the top of
  the page covers every repository by default; pick a subset there to scope
  every metric below it.
:::
