...
- **Job name**: CI Insights records the job a test ran in as part of that
  ...
  Set the plugin's `job_name` option to report a different one:
  ...
  The plugin exports `MERGIFY_TEST_JOB_NAME` itself before it uploads, from
  `job_name` if you set it and from the step label otherwise, so a value put in
  the step's `env` is overwritten. That variable is still the way to rename the
  job when you upload without the plugin, from a framework with a native
  Mergify integration.
