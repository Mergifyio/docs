---
title: CI Insights Setup – GitHub Actions
...
2. To upload your test results, you'll need to [configure your test
   framework](/test-insights#test-framework-configuration). Before doing so,
   create an application key with `ci` scope as described in the [API Usage
   documentation](/api/usage) and set it as `MERGIFY_TOKEN` in your GitHub
   Actions secrets.
   ...
   :::tip
     ...
     variables → Dependabot`). Dependabot workflows run in a restricted
     environment and cannot access regular GitHub Actions secrets.
   :::
...
jobs:
  test:
    ...
    steps:
      ...
      # Upload all test results to Test Insights
...
- `MERGIFY_TOKEN`: Application key with `ci` scope for uploading test results to Test Insights
- `RUN_COUNT`: Number of times to execute each test within a single job
- `MERGIFY_TEST_JOB_NAME`: Override for the job name reported with your test results
