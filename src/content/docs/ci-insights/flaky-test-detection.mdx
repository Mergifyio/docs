...
A **flaky test** is one that fails on some runs of a commit (SHA1) and passes
on others. If a test runs twice on the same commit and once fails while the
other succeeds, it's considered flaky because the outcome is not consistent
with the same code. A test whose results differ without ever failing, such as
one that is skipped in one run and passes in another, stays healthy.
...
for the same test, on the same commit, in the same pipeline and job. When two
results disagree, Mergify records the disagreement and reports the test as
flaky once one of the two is a failure. Any repeat execution counts: a retried
