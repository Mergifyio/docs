...
description: Run CI Insights, Job Selection, and Merge Queue Scopes on Buildkite.
---
...
CI Insights, Job Selection, and Merge Queue Scopes. Any Buildkite status check
...
- **[Job Selection](#job-selection)**: skip unaffected steps on pull requests to cut
...
Job Selection is [scopes](/merge-queue/scopes) applied to your pipelines. The same
...
[GitHub Actions integration](/integrations/gha#job-selection) covers the same
...
```dot class="queue" title="detect-scopes runs first, and each test step runs only when its scope changed"
digraph {
  PR [label="Pull request\nchanges", class="external"];
  Config [label="Scopes config\n(.mergify.yml)", class="config"];
  Detect [label="detect-scopes step\n(mergify-ci plugin)", class="pending"];
...
The test steps the dynamic pipeline emits need the token too, and they can run
on different agents from the one that detected the scopes.
...
- The `detect-scopes` step calls the `mergifyio/mergify-ci` plugin with the
  ...
  `mergify-ci.scopes`. It also annotates the build with the scopes it found, so
  they are readable on the build page without opening the step log.
