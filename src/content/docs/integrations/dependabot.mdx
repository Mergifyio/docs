...
You can automatically merge Dependabot PRs using
[`auto_merge`](/merge-protections/auto-merge) in `merge_protections_settings`.
When all merge protection `success_conditions` pass, the PR is automatically
merged (or queued if the [merge queue](/merge-queue) is enabled).
...
merge_protections:
  - name: dependabot
    ...
    success_conditions:
      # Replace "ci" with the name of your CI check
      - check-success = ci
```
...
`dependabot-update-type`. You can use these in your merge protection conditions
to filter which Dependabot PRs to auto-merge. For instance, you might only want
to auto-merge minor version bumps:
