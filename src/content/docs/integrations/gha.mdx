...
description: Run CI Insights, Job Selection, and Merge Queue Scopes on GitHub Actions.
---
import { Image } from "astro:assets"
import ghaLogo from "../../images/integrations/gha/logo.png"
import ghaSummaryScreenshot from "../../images/integrations/gha/job-selection-summary.png"
...
CI Insights, Job Selection, and Merge Queue Scopes. Any GitHub Actions job status
...
- **[Job Selection](#job-selection)**: skip unaffected jobs on pull requests to cut
  CI spend in monorepos.

- **[Merge Queue Scopes](/merge-queue/scopes)**: run only the jobs
...
{/* Keeps links to the section's former name, Monorepo CI, landing here. */}
<span id="monorepo-ci"></span>

## Job Selection

Job Selection is [scopes](/merge-queue/scopes) applied to your workflows. The same
...
[Buildkite integration](/integrations/buildkite#job-selection) covers the same
ground. Scopes themselves are CI-agnostic, so for any other provider
[let us know](mailto:support@mergify.com) and see
[other build tools](/merge-queue/scopes/others).
...
1. **Detect scopes** using the
   [`gha-mergify-ci`](https://github.com/Mergifyio/gha-mergify-ci) action. Its
   inputs and outputs are listed in the
   [action reference](/integrations/gha/reference).
...
```dot class="queue" title="detect-scopes runs first, and each test job runs only when its scope changed"
digraph {
  PR [label="Pull request\nchanges", class="external"];
  Config [label="Scopes config\n(.mergify.yml)", class="config"];
  Detect [label="detect-scopes job\n(gha-mergify-ci)", class="pending"];
  ...
  Detect -> API [label="scope: api (false)", class="muted", style=dashed];

  Gate [label="ci-gate\n(optional)", class="external"];
  Frontend -> Gate;
  Docs -> Gate;
}
```
...
:::caution
  On a `pull_request` event the `scopes` action works out the base and head to
  diff on its own. Any other event has no pull request to take them from, so
  either pass an explicit [`base`](/integrations/gha/reference#input-base), or
  guard the job with `if: ${{ github.event_name == 'pull_request' }}` and skip
  it. Sending the scopes to Mergify is a no-op outside a pull request either
  way: on other events the action only writes its `scopes` output.
:::
...
The `token` on `detect-scopes` is a Mergify application key with the [`ci`
scope](/api/usage#application-key-scopes), stored as a repository secret; an
`admin` key is rejected. It lets the action send the scopes it detected back to
Mergify, which is how [Merge Queue Scopes](/merge-queue/scopes) know what a
queued pull request affects. `wait-jobs` needs no token. For pull requests
opened by Dependabot, add the same secret to your [Dependabot
secrets](/integrations/dependabot#configuring-mergify_token-for-dependabot) as
well.
