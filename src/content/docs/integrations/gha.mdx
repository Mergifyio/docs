...
:::caution
  ...
  either pass an explicit `base`, or guard the job with
  `if: ${{ github.event_name == 'pull_request' }}` and skip it. Sending the
  scopes to Mergify is a no-op outside a pull request either way: on other
  events the action only writes its `scopes` output.
:::
