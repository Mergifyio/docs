...
2. A Mergify application key with the `admin` scope, or a GitHub personal
   access token. See [Authentication](#authentication) below.
...
An application key must hold the `admin`
[scope](/api/usage#application-key-scopes). A `ci` key only reaches the
CI-facing endpoints, so the endpoints behind the resources below reject it with
`403`. That is easy to hit, because `MERGIFY_TOKEN` is also the variable a CI
pipeline carries its `ci` key in.
...
```hcl
resource "mergify_repository_products" "hello_world" {
  owner      = "octo"
