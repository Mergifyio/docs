...
2. **Enter Command in the Comment Section**: In the comment box, type the
   ...
   `@Mergify` works too, and neither prefix is case-sensitive, so
   `@mergifyio rebase` and `@Mergifyio rebase` are the same command.

   Mergify only reads a command that starts the comment, with a single space
   between `@Mergifyio` and the command name. `Please @Mergifyio rebase` runs
   nothing, and neither does a command written after a line of text. Arguments
   go on that same line; anything on the lines below is left alone.
