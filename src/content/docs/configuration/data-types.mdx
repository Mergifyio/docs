...
`{{ author }}` variable to act as the pull request author.

It has to be a user account. Mergify cannot impersonate an account owned by
another GitHub App, so an action fails when the login it resolves to is a bot,
including when no `bot_account` is set and it falls back to the pull request
author or to whoever posted a command. Used by the
...
:::note
  ...
  substitution](#variable-substitution). Accounts that installed Mergify
  recently never had it at all: their configuration is read as if Jinja2 did
  not exist, so anything beyond the variables a field lists is ordinary text
  there, accepted when the configuration is validated and then rendered
  verbatim. This applies to the hosted service only; on a self-hosted
  installation Jinja2 still renders whenever the account was created.
:::
