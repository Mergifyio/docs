---
title: Sharing Configuration
description: Learn how to share and reuse configuration pieces in your configuration.
---

In Mergify, you can use features like YAML anchors and aliases,
the `shared` key, and the `extends` key to reduce redundancy and keep your
configuration consistent.

## YAML Anchors and Aliases

[YAML anchors (`&`) and aliases
(`*`)](https://yaml.org/spec/1.2.2/#anchors-and-aliases) are supported in the
[configuration file](/configuration/file-format). These allow you to reuse
configuration sections. For example, you can create a list of continuous
integration checks and use it in multiple sections:

```yaml
queue_rules:
  - name: hotfix
    queue_conditions:
      - and: &CheckRuns
        - check-success = linters
        - check-success = unit
        - check-success = functional
        - check-success = e2e
        - check-success = docker
  - name: default
    queue_conditions:
      - and: *CheckRuns
      - schedule = Mon-Fri 09:00-17:30[Europe/Paris]
```

## Sharing Configuration Sections

You can store anything in the `shared` key. Its main purpose is to provide a
place to put redundant YAML anchors:

```yaml
shared:
  my_ci: &common_checks
    - check-success = ci-one
    - check-success = ci-two

queue_rules:
  - name: hotfix
    queue_conditions:
      - and: *common_checks
  - name: default
    queue_conditions:
      - and: *common_checks
      - schedule = Mon-Fri 09:00-17:30[Europe/Paris]
```

## Extending Configuration Files

Mergify lets you extend your configuration file by including the configuration
from another repository. This feature is useful when you want to share and
reuse configuration rules across multiple repositories, helping maintain
consistency and reduce duplication.

To extend a configuration file, use the `extends` keyword at the top level of
your configuration file and specify the repository name from which you
want to include the configuration. The value is the repository name on its own,
not `owner/repo`: the repository is looked up in the same organization.

```yaml
extends: shared-config
```

:::caution
  - The Mergify application must also be installed on the source repository
    that you want to import the configuration from.

  - A configuration cannot extend itself, and the source repository's own
    configuration cannot use `extends`: only one level of extension is
    supported.

  - The source repository must be at least as visible as yours, since its
    configuration is displayed wherever your repository is readable. A private
    or internal repository can extend a public one, and a private repository
    can extend an internal one, but never the other way around. See GitHub's
    documentation on [repository
    visibility](https://docs.github.com/en/repositories/creating-and-managing-repositories/about-repositories#about-repository-visibility).

  - Visibility is checked every time the configuration is loaded. Making your
    repository more visible, or the source repository less visible, breaks your
    configuration even though no configuration file changed.

  - When a pull request points `extends` at a repository your committed
    configuration does not already extend, Mergify resolves it only if the
    change comes from a branch of your repository or from an author with write
    access to it. Otherwise it declines, and the [configuration
    check](/configuration/file-format#validation) reports that the new
    configuration could not be validated. The target the committed
    configuration already extends always resolves.
:::

The configuration from the specified repository will be loaded and applied
before the one in the current repository. Here's an example:

```yaml
extends: shared-config

pull_request_rules:
  - name: additional rule specific to this repository
    conditions:
      - base = main
    actions:
      label:
        add:
          - repository-specific
```

:::caution
  - Values in the `shared` key will not be merged and shared between local and
    remote configurations.

  - Values in the [`scopes`](/merge-queue/scopes) key will not be merged either:
    scopes defined in the extended configuration do not apply to the local
    configuration.

  - Values in the `defaults`, `commands_restrictions`, `merge_queue`, and
    `merge_protections_settings` keys will be merged and remote default values
    will apply to the local configuration, unless the same value already exists
    in the local configuration.

  - Rules in the `pull_request_rules`, `queue_rules`, `priority_rules` or
    `merge_protections` section will be merged and remote default values will
    apply to the local configuration. A rule will not be overwritten by the
    extended configuration if it has the same name in both the extended and
    local configuration.
:::
