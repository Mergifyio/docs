...
To drop a command's default restriction altogether, set its `conditions` to
`true`. That's shorthand for an empty list of conditions, which nothing can
fail, so anyone who can comment on the pull request can run the command:
