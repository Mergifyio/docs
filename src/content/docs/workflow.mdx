...
defined. This behavior is known as "edge triggering": once an action has run,
Mergify does not run it again while the rule keeps matching. It runs a second
time only after the pull request stops matching the rule and then matches again.

Not every action works that way. `assign`, `dismiss_reviews`, `edit`, `label`,
`post_check`, `rebase`, `request_reviews`, `review`, `squash`, and `update` run
on every evaluation instead, which is how they hold a pull request in the state
a rule describes. The rule above relies on that: if someone removes the `WIP`
label while the title still matches, Mergify puts it back.
