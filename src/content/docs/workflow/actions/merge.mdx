...
- Mergify never merges a pull request that is a draft, closed, or in conflict

- Mergify always respects the [branch protection settings](#branch-protection-conditions)

- Mergify always respects your [Merge Protections](/merge-protections), when
  they are enabled on the repository
...
- Mergify merges pull requests that modify its own configuration file, but only
  once its [configuration check](/configuration/file-format#validation) has
  passed, so a broken configuration is never merged

- With `method: fast-forward`, Mergify also waits for the pull request to be up
  to date with its base branch (`#commits-behind = 0`), since a fast-forward
  merge cannot move the base branch otherwise

:::note
  A [scheduled freeze](/merge-protections/freeze) decides the `Mergify Merge
  Protections` check only when the [merge queue](/merge-queue) is disabled.
  When the merge queue is enabled, the freeze is reported in that check but
  enforced by the queue itself, so the check can pass while a freeze is
  running and the `merge` action goes ahead. Queue the pull request with the
  [`queue` action](/workflow/actions/queue) instead if you want the freeze to
  hold it back.
:::
...
injects them as merge conditions. For example, a rule requiring an approved
review injects the
[`github-review-approved`](/configuration/conditions#github-rulesets-and-branch-protection-attributes)
condition. That condition reproduces GitHub's own review decision rather than
counting approvals, so an active "Request changes" from a reviewer other than
the approver keeps it unsatisfied.
