...
That check applies to people: a review left by a GitHub App counts regardless of
the permission the app holds. GitHub Copilot is the exception in the other
direction, since Mergify ignores its reviews wherever reviews are counted, so a
Copilot review never marks a pull request as already reviewed.
