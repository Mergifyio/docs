...
matches `release-10`. Setting both `branches` and `regexes` backports to the
...
In this configuration, a pull request is backported when it has the label
`backport`. Then, when the backport is created and passes the check named
`continuous-integration`, it will be automatically merged. Mergify opens each
backport from a head branch named
`mergify/bp/<destination-branch>/pr-<original-number>`, which is what the `head`
condition matches. The base of the backport is the destination branch itself.
