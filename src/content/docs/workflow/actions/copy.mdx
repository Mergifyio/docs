...
`regexes` picks destination branches by
[pattern](/configuration/data-types#regular-expressions) instead of naming each
one. Patterns are matched against the branches that exist in the repository, and
each one is anchored at the start of a branch name only, so `release-1` also
matches `release-10`. Setting both `branches` and `regexes` copies to the
branches you named plus the branches your patterns matched.
