...
The update action checks one more thing, and checks it before any of the above.
When your repository is public, Mergify refuses to update a pull request from a
fork whose author has not ticked GitHub's ["Allow edits from
...
box, and the check run asks them to tick it. The box is not required when your
repository is private. A queue using in-place checks requires it whatever the
visibility, when a pull request asks to be queued.
