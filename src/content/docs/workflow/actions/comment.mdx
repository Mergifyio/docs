...
`message` defaults to `null`, and a `comment` action without one is an invalid
configuration. Set it on the action or in
[`defaults`](/configuration/file-format#defaults).
