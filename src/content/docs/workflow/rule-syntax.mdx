...
Mergify checks every rule against the pull request, and matching does not stop at
the first hit: each rule whose conditions match runs its actions.

Rule order still decides one thing. `close`, `delete_head_branch`, `merge`,
`queue`, `rebase`, `squash` and `update` refuse to run twice in the same
evaluation. When two matching rules both carry one of them, the rule listed
first in the file runs it, and the later rule's check reports that another
action of that kind already ran. Put the rule you want to win above the others.

For when an action re-runs versus stays put, see
