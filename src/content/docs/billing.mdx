...
Both counts are clickable: the repository count takes you to your repositories
list, and the contributor count opens the list of people counted for that
product.
...
A single invoice usually mixes two things: an adjustment for the period that
just ended, and the charge for the period that is starting. The adjustment is
there because the number of contributors Mergify counts can change any day.
Cancelling your subscription produces one more invoice, which works differently
and is covered last.

### 1. **Real-time User Adjustment**

Mergify keeps the number of seats on your subscription in step with the number
of contributors it counts. Every time that count changes, the adjustment is
prorated: you are credited for the seats you had already paid for over the rest
of the period, then charged for the new count over those same days. Both lines
show up on your next invoice, labeled "unused time" and "remaining time" by our
payment provider, so only the difference between them changes your total.

### 2. **Upcoming Period Charge**

The rest of the invoice charges the period that is about to start, based on
your user count on the day it is issued. If that count changes during the
period, the next invoice carries the same kind of adjustment.

### A Worked Example

Say `example-org` is billed on the 1st of each month at $10 per user per month
and has 20 contributors. Two more contributors push commits on April 16th,
halfway through the period. The invoice issued on May 1st then reads:

| Line | Period | Amount |
|---|---|---|
| Unused time on 20 users | Apr 16 – May 1 | -$100.00 |
| Remaining time on 22 users | Apr 16 – May 1 | $110.00 |
| 22 users | May 1 – Jun 1 | $220.00 |
| **Total** | | **$230.00** |

The first two lines are the real-time user adjustment: half of April is
credited back at the old count, and the same half is re-charged at the new one.
The $10 difference is what the two extra contributors cost for the time they
were counted. The third line is the upcoming period, at the count you have on
the day the invoice is issued.

:::note
  `example-org`, the 20 contributors, and the $10 per-user rate are all made up
  to keep the arithmetic easy to follow. Your own invoice uses the rate on your
  plan, and your billing dates come from your own subscription.
:::
