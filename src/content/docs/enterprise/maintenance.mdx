...
[Releases](/enterprise/releases) lists every version and what moving to it involves.
...
Restore the database before starting the container: the container applies any pending schema
migration on startup, so PostgreSQL must be reachable and holding the restored data by then.
