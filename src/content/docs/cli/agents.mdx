...
repository under `skills/`, are Apache-2.0, and ship as one bundle. For Codex,
they're published as the
[Mergify listing](https://chatgpt.com/plugins/plugins_6aabe2c8f7b88191a6962623c8199c7b)
in the OpenAI plugin directory; every agent, Codex included, can also install
them through skills.sh.
...
Codex and ChatGPT share one plugin directory, but the
[Mergify listing](https://chatgpt.com/plugins/plugins_6aabe2c8f7b88191a6962623c8199c7b)
in it currently only works from Codex — the skills shell out to `mergify`,
which ChatGPT itself cannot run. Open the plugin directory from Codex and
search it for Mergify:

```text
codex /plugins
```

### Cursor, Cline, Copilot, and other skills.sh agents

[skills.sh](https://skills.sh) installs the same bundle into Cursor, Cline,
Copilot, and dozens of other agents (Codex included, if you'd rather install
it this way than through the plugin directory above):
...
```bash
npx skills add Mergifyio/mergify-cli --agent cursor --global
```
...
An `admin` key covers queue status, the Activity Log, freezes, and browsing or
quarantining tests; CI Insights uploads and Merge Queue Scopes need a `ci` key
instead. No single key covers
...
stacks needs a GitHub token, which is not deprecated: the deprecation above is
only about using a GitHub token to reach the Mergify API. Exporting
`GITHUB_TOKEN` is not enough on its own, though. Stacks commands read `--token`,
then `MERGIFY_TOKEN`, then `GITHUB_TOKEN`, so an agent that already has an
application key in `MERGIFY_TOKEN` hands that key to GitHub and the push fails.
Pass the GitHub token as `--token` on the Stacks commands, or keep the
application key out of `MERGIFY_TOKEN`. See
[GitHub token for Stacks](/cli/usage#github-token-for-stacks) for the full
resolution order.
