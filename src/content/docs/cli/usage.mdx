...
of its own. The [`auth` command reference](/cli/auth) lists the options each
`mergify auth` command takes.
...
The `mergify tests` commands go the other way. Apart from
`mergify tests quarantines list`, the endpoints behind them refuse a `ci` key
and answer `403`, so they need an `admin` key, a GitHub personal access token,
or a user token.
...
| `8` | `ConfigurationError` | Configuration or credentials missing, unparseable, or failing validation. |
