...
Downstream jobs then declare `needs: detect-scopes` and gate on those outputs, for example
`if: ${{ needs.detect-scopes.outputs.backend == 'true' }}`.

:::tip
  Did you know? Scopes also work outside of the merge queue to optimize your
  CI pipelines. See the [GitHub Actions integration](/integrations/gha#job-selection)
...
referenced `partition-name` or `current-partition-name` anywhere in your
conditions, remove those conditions as well. Unlike the `partition_rules`
section, which is ignored, neither is a condition attribute any more, so a
single one left behind makes the whole configuration invalid.

If Mergify has already opened an automatic migration pull request for your
repository, merging it removes the `partition_rules` section for you.
