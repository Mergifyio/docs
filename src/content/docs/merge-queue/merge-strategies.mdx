...
| `fast-forward` | Linear | Original commits moved to base | Yes | Full |
...
behavior depends on whether the queue operates in **inplace mode** or
**draft-PR mode**.

### Inplace Mode

When `batch_size` is `1`, a single parallel check is used, and two-step CI is
not enabled, Mergify operates in inplace mode: the PR is rebased on top of the
base branch, CI runs on the PR itself, and the base branch ref is
fast-forwarded to the PR's head commit.
...
When batching is enabled (`batch_size > 1`), multiple parallel checks are
used (`max_parallel_checks > 1`), or two-step CI is configured, Mergify
creates a draft PR that combines the queued pull requests, runs CI on that
draft, and then fast-forwards the base branch to the draft PR's head commit.
