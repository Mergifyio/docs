---
title: Merge Strategies
description: Choose how pull requests are merged into your base branch and control the shape of your git history.
---

import { Image } from "astro:assets"
import requiredPRbypassScreenshot from "../../images/merge-queue/batches/mergify-required-pull-request-bypass.png"
import GitGraph from '~/components/GitGraph.astro'

The `merge_method` option in your queue rules controls how Mergify merges pull
requests into your base branch. Each method produces a different git history
shape, with trade-offs between linearity, SHA preservation, and throughput.

## Merge Methods at a Glance

| Method | History | Commits on base branch | SHAs preserved | Queue parallelism |
|---|---|---|---|---|
| `merge` | Non-linear | Original commits + merge commit | Yes | Full |
| `squash` | Linear | 1 new commit per PR | No | Full |
| `rebase` | Linear | Recreated copies of each commit | No | Full |
| `fast-forward` | Linear | Original commits moved to base | Yes | Serial only |
| `merge-batch` | Non-linear | Original commits + 1 merge commit per PR + 1 per batch | Yes | Full |

## Merge

```yaml
queue_rules:
  - name: default
    merge_method: merge
```

Creates a merge commit joining the PR branch into the base branch. This is the
default GitHub merge behavior. When you leave `merge_method` unset, Mergify uses
the first method your repository allows, so a repository that disallows merge
commits falls back to squash or rebase instead.

<GitGraph
  nodes={[
    { id: "A", label: "A", role: "external" },
    { id: "B", label: "B", role: "external" },
    { id: "C", label: "C", role: "queued" },
    { id: "D", label: "D", role: "queued" },
    { id: "M", label: "M", role: "merged" },
  ]}
  edges={[
    { from: "A", to: "B" },
    { from: "B", to: "C" },
    { from: "C", to: "D" },
    { from: "B", to: "M" },
    { from: "D", to: "M" },
  ]}
  branch={{ label: "main", node: "M" }}
/>

- **History:** non-linear; the PR branch and base branch are visible as
  separate lines in `git log --graph`

- **Merge commits:** yes; each PR produces a merge commit on the base branch

- **SHAs preserved:** yes; original PR commits keep their SHAs

- **Use case:** most teams; simplest setup with no constraints on parallelism
  or batching

## Squash

```yaml
queue_rules:
  - name: default
    merge_method: squash
```

Squashes all PR commits into a single commit on the base branch.

<GitGraph
  nodes={[
    { id: "A", label: "A", role: "external" },
    { id: "B", label: "B", role: "external" },
    { id: "S", label: "S", role: "pending" },
    { id: "C", label: "C", role: "queued", ghost: true },
    { id: "D", label: "D", role: "queued", ghost: true },
  ]}
  edges={[
    { from: "A", to: "B" },
    { from: "B", to: "S" },
    { from: "B", to: "C", dashed: true },
    { from: "C", to: "D", dashed: true },
    { from: "D", to: "S", dashed: true, label: "squashed\ninto one commit" },
  ]}
  branch={{ label: "main", node: "S" }}
/>

- **History:** linear; one commit per PR on the base branch
- **Merge commits:** no
- **SHAs preserved:** no; a new commit is created
- **Use case:** teams that want a clean `git log` where one commit = one PR

## Rebase

```yaml
queue_rules:
  - name: default
    merge_method: rebase
```

Replays each PR commit on top of the base branch, creating new commits with new
SHAs.

<GitGraph
  nodes={[
    { id: "A", label: "A", role: "external" },
    { id: "B", label: "B", role: "external" },
    { id: "C", label: "C", role: "queued", ghost: true },
    { id: "D", label: "D", role: "queued", ghost: true },
    { id: "Cp", label: "C'", role: "pending" },
    { id: "Dp", label: "D'", role: "pending" },
  ]}
  edges={[
    { from: "A", to: "B" },
    { from: "B", to: "Cp", label: "replayed with\nnew SHAs" },
    { from: "Cp", to: "Dp" },
    { from: "A", to: "C", dashed: true },
    { from: "C", to: "D", dashed: true },
  ]}
  branch={{ label: "main", node: "Dp" }}
/>

- **History:** linear; no merge commits, individual commits are preserved

- **Merge commits:** no

- **SHAs preserved:** no; commits are recreated with new SHAs, so the PR
  branch ref won't match the base branch

- **Use case:** teams that want linear history with individual commits visible,
  but don't need SHA preservation

## Fast-Forward

```yaml
queue_rules:
  - name: default
    merge_method: fast-forward
```

Fast-forward merging advances the base branch ref directly to the tested
commit(s) using the Git API instead of creating a merge via GitHub. The exact
behavior depends on whether the queue operates in
[inplace or batch-PR mode](/merge-queue/batches#in-place-checks-no-batch-prs).

### Inplace Mode

The PR is rebased on top of the base branch, CI runs on the PR itself, and
the base branch ref is fast-forwarded to the PR's head commit.

In this mode, `update_method` must be set to `rebase` (the default). If a
rebase update occurs, the commit SHAs on the PR will change; what fast-forward
preserves are the SHAs of the PR branch at merge time.

:::caution
  Combining fast-forward inplace mode with an `update_bot_account` makes
  Mergify refuse pull requests coming from a fork when they are queued.
  `update_method: merge` is not a way out here: a fast-forward queue lands by
  moving the base branch onto the pull request's head, and it exists to keep
  merge commits out of that history. See [Combining Merge and Update
  Methods](#combining-merge-and-update-methods) for what to do instead.
:::

<GitGraph
  nodes={[
    { id: "A", label: "A", role: "external" },
    { id: "B", label: "B", role: "external" },
    { id: "C", label: "C", role: "queued" },
    { id: "D", label: "D", role: "queued" },
  ]}
  edges={[
    { from: "A", to: "B" },
    { from: "B", to: "C", label: "same SHAs\non base branch" },
    { from: "C", to: "D" },
  ]}
  branch={{ label: "main", node: "D" }}
/>

- **History:** strictly linear; commits sit directly on the base branch

- **Merge commits:** none

- **SHAs preserved:** yes; the exact same commit SHAs from the PR appear on
  the base branch

### Batch-PR Mode

Mergify creates a batch PR that combines the queued pull requests, runs CI on
that batch, and then fast-forwards the base branch to the batch PR's head
commit.

Because the batch branch combines multiple PRs, the history **may contain merge
commits** from folding each PR into the batch branch.

- **History:** linear at the base-branch level, but individual merge commits
  from combining PRs may appear

- **Merge commits:** possible (from combining PRs in the batch branch)

- **SHAs preserved:** the final merged result is the exact SHA tested by CI

### Constraints

- **`commit_message_format` has no effect**: fast-forward preserves
  the original commits, so custom commit messages are not applicable

- **[Parallel and isolated modes](/merge-queue/queue-modes) are not
  supported**: both merge batches independently of each other, which
  fast-forward cannot do. Mergify rejects a queue rule that combines them

- **Use case:** teams and OSS projects that care about commit identity and want
  the exact CI-tested code on their base branch

:::caution
  Fast-forward requires Mergify to push directly to the base branch without
  going through a pull request merge. If GitHub branch protections are enabled,
  you must allow Mergify to **bypass the required pull requests** setting.
  See [GitHub Rulesets Compatibility](/merge-queue/github-rulesets) for
  setup instructions.

  <Image src={requiredPRbypassScreenshot} alt="Mergify bypass required pull requests" />
:::

### Examples

Inplace mode, with strictly linear history and preserved commit SHAs:

```yaml
merge_queue:
  max_parallel_checks: 1

queue_rules:
  - name: default
    merge_method: fast-forward
    batch_size: 1
    merge_conditions:
      - check-success = ci
```

Batch-PR mode, combining batching with fast-forward merge:

```yaml
queue_rules:
  - name: default
    merge_method: fast-forward
    batch_size: 5
    merge_conditions:
      - check-success = ci
```

## Merge Batch

```yaml
queue_rules:
  - name: default
    merge_method: merge-batch
    batch_size: 5
    merge_conditions:
      - check-success = ci
```

When `merge_method` is set to `merge-batch`, Mergify merges the batch pull
request it creates for batching directly into the base branch using a merge
commit, instead of merging the original pull requests individually.

This is similar to `fast-forward` in that the batch PR itself is merged, but
uses the GitHub pull request merge API rather than advancing the git ref
directly. The merge commit that lands on the base branch is titled after the
pull requests included in the batch (for example, `Merged #42, #43, #44`), with
`Merged by Mergify Merge Queue` as its body. The merges inside the batch branch
carry a title of their own, `Merge of #42`, one per pull request.

<GitGraph
  nodes={[
    { id: "A", label: "A", role: "external" },
    { id: "B", label: "B", role: "external" },
    { id: "C1", label: "C1", role: "queued" },
    { id: "C2", label: "C2", role: "queued" },
    { id: "D1", label: "D1", role: "batch" },
    { id: "D2", label: "D2", role: "batch" },
    { id: "M", label: "M", role: "merged" },
  ]}
  edges={[
    { from: "A", to: "B" },
    { from: "B", to: "C1" },
    { from: "B", to: "C2" },
    { from: "B", to: "D1", label: "batch PR" },
    { from: "C1", to: "D1", label: "Merge of #42" },
    { from: "D1", to: "D2" },
    { from: "C2", to: "D2", label: "Merge of #43" },
    { from: "B", to: "M" },
    { from: "D2", to: "M", label: "merge commit" },
  ]}
  branch={{ label: "main", node: "M" }}
/>

- **History:** non-linear; a single merge commit per batch appears on the base
  branch

- **Merge commits:** yes; one per batch on the base branch, plus one per PR
  inside the batch branch

- **SHAs preserved:** yes; Mergify builds the batch branch by merging each
  embarked pull request's head into it, so that head commit becomes the second
  parent of a `Merge of #42` commit instead of a copy. Only the merge commits
  are new

- **Requires `batch_size > 1`**: this method is designed for batch merging and
  cannot be used with single PRs

- **Use case:** teams that use batch merging and want a single merge commit per
  batch on the base branch, triggering only one deployment per batch

:::note
  Unlike `fast-forward`, `merge-batch` uses the standard GitHub pull request
  merge API, so it works with branch protection settings that require pull
  request merges. No bypass configuration is needed.
:::

## Editing the Batch Branch

:::danger
  `allow_queue_branch_edit` is **deprecated** and will be removed in a future
  release. There is no replacement: move the work now done on a batch branch
  onto the pull requests themselves. Once the option is gone, an edited batch
  branch dequeues the pull requests embarked in it and reports the edit as a
  failure, which is what a queue that leaves the option unset already does.
:::

By default, if anything other than Mergify pushes a commit to the branch of a
[batch PR](/merge-queue/batches), Mergify dequeues the pull requests embarked in
it and reports the failure. Setting `allow_queue_branch_edit: true` on a queue
rule tells Mergify to trust the branch instead and keep going.

:::caution
  `allow_queue_branch_edit` is deprecated and will be removed in a future
  release, with no replacement. Once it is gone, every queue behaves the way a
  queue that leaves the option unset behaves today: an edited batch branch
  dequeues the pull requests it carries and the edit is reported as a failure.
  Move the work you do on a batch branch onto the pull requests themselves.
:::

That option requires `merge_method: merge-batch` or `merge_method: fast-forward`.
Those are the only two methods that merge the batch branch itself, so they are
the only ones that can land a commit pushed to it. With `merge`, `squash` or
`rebase`, each pull request is merged at its own head, the batch branch is never
an input to the merge, and such a commit is dropped without a trace. Mergify
rejects the configuration rather than letting the option look like it is doing
something. Set `merge_method` explicitly: leaving it unset is rejected as well,
because the repository default it resolves to is never `merge-batch` or
`fast-forward`.

```yaml
queue_rules:
  - name: default
    merge_method: merge-batch
    batch_size: 5
    allow_queue_branch_edit: true
```

:::caution
  Under these two merge methods, what lands on the base branch is the batch
  branch as a whole. Enabling this option therefore means merging commits that
  were never reviewed as part of any pull request. That is what the option is
  for: batch-level work such as a conflict resolution or a lockfile regeneration
  that belongs to no single pull request. Make sure only Mergify and your own
  tooling can push to these branches.
:::

An accepted edit also restarts the batches behind the edited one, whenever
[parallel checks](/merge-queue/performance#parallel-checks) put more than one batch in flight. Each
of those batches was composed on top of what the batches ahead were expected to merge, so from the
edit onwards their checks run against a tree that will never exist. Mergify tears them down,
recomposes them on the edited head and starts their checks again; the pull requests stay in the queue
throughout. The edited batch itself keeps going, since its own checks already run on the head that
was pushed. Discarding the running checks behind it is the cost of editing a batch branch that has
work queued after it.

Mergify also does not mark the embarked pull requests as merged one by one here.
GitHub considers a pull request merged only once its head commit is reachable
from the new base branch tip, so a pull request whose head no longer matches what
was embarked is not shown as merged even though its content landed.

## Combining Merge and Update Methods

The `update_method` option controls how Mergify updates PR branches when they
fall behind the base branch. Combining `merge_method` with `update_method`
gives you additional control over your history shape.

:::caution
  Mergify refuses a pull request coming from a fork at queue entry if the queue
  [runs its checks in place](/merge-queue/batches#in-place-checks-no-batch-prs)
  with `update_method: rebase` and an `update_bot_account`. GitHub refuses an
  OAuth token on its rebase API for a fork, so rebasing on behalf of a user
  would mean impersonating that user to force-push the contributor's branch,
  and Mergify does not do that.

  Unset `update_bot_account` to keep queueing pull requests from forks: with no
  account to impersonate, the update rebases through GitHub's API as Mergify.
  On a queue that does not merge with `fast-forward`, `update_method: merge`
  works too, and merges the base branch into the pull request rather than
  rebasing it.

  One kind of pull request from a fork goes the other way: for one that
  changes GitHub Actions workflows, or that another GitHub App's bot opened,
  GitHub lets Mergify update in place only *with* an `update_bot_account`. A
  queue that receives those has to run its checks on a
  [draft pull request](/merge-queue/batches) instead.
:::

### Semi-Linear History (Rebase + Merge Commit)

```yaml
queue_rules:
  - name: default
    merge_method: merge
    update_method: rebase
```

PRs are rebased on top of the base branch before being merged with a merge
commit. This produces a history where individual commits are linear, but each
PR is wrapped in a merge commit that marks the PR boundary.

- **Use case:** teams that want linear commits but also want merge commits as
  PR boundary markers in `git log --graph`

### Linear History via Rebase Update

```yaml
queue_rules:
  - name: default
    merge_method: rebase
    update_method: rebase
```

PRs are rebased to stay current, then rebased again at merge time. This
produces a fully linear history with no merge commits, though SHAs will differ
from the original PR branch.

## Choosing the Right Strategy

| I want... | `merge` | `squash` | `rebase` | `fast-forward` | `merge-batch` |
|---|:---:|:---:|:---:|:---:|:---:|
| Linear history | | ✔ | ✔ | ✔ | |
| Preserved commit SHAs | ✔ | | | ✔ | ✔ |
| One commit per PR | | ✔ | | | |
| One merge commit per batch on the base branch | | | | | ✔ |
| Individual PR commits visible | ✔ | | ✔ | ✔ | ✔ |
| See PR boundaries in `git log` | ✔ | | | | ✔ |
| [Batches](/merge-queue/batches) and [parallel checks](/merge-queue/performance#parallel-checks) | ✔ | ✔ | ✔ | | ✔ |
| Single deployment per batch | | | | ✔ | ✔ |
