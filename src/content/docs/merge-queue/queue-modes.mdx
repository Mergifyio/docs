...
`scopes.default_capacity` sets the limit for every scope you didn't list in `capacities`. Naming each
...
carries gets three. An explicit `capacities` entry always wins for its own scope, so you only name
the scopes that need something other than the default.
