...
in all three a pull request whose scopes are untouched by the base branch can skip its CI run
entirely through [Direct Merge](/merge-queue/direct-merge). They differ only in **how batches depend
on each other**.
