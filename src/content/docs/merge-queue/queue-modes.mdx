...
All three honor your [queue rules](/merge-queue/rules) and [batch sizing](/merge-queue/batches), and
in all three [Direct Merge](/merge-queue/direct-merge) can skip a pull request's CI run entirely
when the base branch has not moved underneath it, either not at all or only in scopes it does not
touch. They differ only in **how batches depend on each other**.
...
```dot class="queue" title="Serial mode — one queue: batch 1 is being tested, batches 2 and 3 wait behind it"
digraph {
  PR1 [label="Batch 1\nPR #1 (api)", class="pending"];
  PR2 [label="Batch 2\nPR #2 (frontend)", class="queued"];
  PR3 [label="Batch 3\nPR #3 (docs)", class="queued"];
  PR1 -> PR2 -> PR3;
}
```
...
```dot class="flow" title="Parallel mode — batches with no scope in common are tested at the same time"
digraph {
  edge [style=invis];
  ...
  subgraph cluster_running {
    ...
    PR1 [label="Batch 1\nPR #1 (api)", class="pending"];
    PR2 [label="Batch 2\nPR #2 (frontend)", class="pending"];
    PR3 [label="Batch 3\nPR #3 (docs)", class="pending"];
...
```dot class="queue" title="Parallel mode — batch 2 shares the api scope with batch 1, so it waits; batch 3 proceeds on its own"
digraph {
  PR1 [label="Batch 1\nPR #1 (api)", class="pending"];
  PR4 [label="Batch 2\nPR #4 (api, frontend)", class="queued"];
  PR3 [label="Batch 3\nPR #3 (docs)", class="pending"];
...
#### Naming scopes with patterns

A key holding `*`, `?`, `[` or `{` is a glob over scope names rather than one name, with the syntax
of [`files` filters](/configuration/data-types#globs): `*` and `?` stay inside one `/`-separated
segment, and only a whole `**` segment crosses them. So `//lib/**` matches the packages below
`//lib` but not `//lib` itself. A pattern gives **every scope it matches its own budget**, not one
budget the matches share.
...
Quote any key starting with `@`, `*`, `?`, `[` or `{`: YAML does not read those as plain scalars.

A key naming a scope exactly wins over any pattern, so `@acme/payments` is held to one check while
its siblings get two. When several patterns match a scope and no key names it, the smallest capacity
applies, so the result never depends on key order, which a YAML mapping does not define anyway.

What an exact key may say depends on [`scopes.source`](/merge-queue/scopes). With a `manual` source
it can be any name the scopes API accepts, including a Bazel label like `//lib/core` or a scoped
package name like `@acme/web`. Under any other source it must be a scope name the `files` source
could declare: at least two characters, matching `^[A-Za-z0-9_-]+$`. A key outside that is rejected
rather than ignored, since it would cap a scope that can never exist.

#### How capacities relate to the global ceiling

`max_parallel_checks` is the **global ceiling**: the most speculative checks a queue will ever run at
...
- A scope that no `capacities` key matches is unlimited; it draws on the global ceiling alone.
  [`default_capacity`](#capping-scopes-you-did-not-list) changes that.
...
```dot class="flow" title="Per-scope capacity — ceiling 5, frontend 2, backend 2, docs uncapped"
digraph {
  edge [style=invis];
  ...
  subgraph cluster_running {
    ...
    F1 [label="frontend #1", class="pending"];
    F2 [label="frontend #2", class="pending"];
    B1 [label="backend #1", class="pending"];
    B2 [label="backend #2", class="pending"];
    D1 [label="docs #1", class="pending"];
  ...
  subgraph cluster_waiting {
    class="batch";
    label="Waiting";
    F3 [label="frontend #3\nfrontend full", class="queued"];
    B3 [label="backend #3\nbackend full", class="queued"];
    D2 [label="docs #2\nceiling full", class="queued"];
    { rank=same; F3; B3; D2; }
  }
}
```
...
#### Capping scopes you did not list

`scopes.default_capacity` sets the limit for every scope no `capacities` key matches. Naming each
scope works when you have a handful of them, but not when they come from a build system: an
[Nx](/merge-queue/scopes/nx) or [Bazel](/merge-queue/scopes/bazel) graph names hundreds of projects,
and the set changes with every pull request that adds one.
...
Here `e2e` is held to one speculative check at a time, and every other scope a queued pull request
carries gets three. Any matching `capacities` key wins for its scope, so you only name the scopes
that need something other than the default.

The default applies to the scopes Mergify sees on the pull requests currently queued, which is what
lets it cover names you never wrote down. Pull requests carrying no scope at all are unaffected: they
draw on the global ceiling alone, as they do without any capacity configured. And
`max_parallel_checks` still caps the total: like `capacities`, a default capacity only ever holds
scopes below the global ceiling, it never raises the number of checks running at once.

Like `capacities`, this option needs `parallel` or `isolated` mode. Setting it on a `serial` queue is
rejected rather than ignored: serial mode tests everything in one cumulative pipeline, so there are
no independent per-scope lanes for a sub-limit to bind on.
...
:::note
  ...
  queue to one check at a time, so each scope's effective capacity becomes 1 and `capacities` has no
...
```dot class="flow" title="Isolated mode — every batch is tested on its own, with no dependency on any other"
digraph {
  edge [style=invis];
