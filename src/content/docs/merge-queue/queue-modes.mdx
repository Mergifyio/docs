...
```dot class="queue"
digraph {
  label="Serial mode — a single queue";
  PR1 [label="Batch 1\nPR #1 (api)", class="queued"];
...
```dot class="flow"
digraph {
  label="Parallel mode — independent scope queues";
  edge [style=invis];
...
```dot class="flow"
digraph {
  label="Per-scope capacity — ceiling 5, frontend 2, backend 2, docs uncapped";
  edge [style=invis];
  ...
  subgraph cluster_waiting {
    class="muted";
    label="Waiting";
    F3 [label="frontend #3\nfrontend full", class="muted"];
    B3 [label="backend #3\nbackend full", class="muted"];
    D2 [label="docs #2\nceiling full", class="muted"];
    { rank=same; F3; B3; D2; }
  }
}
```
...
```dot class="flow"
digraph {
  label="Isolated mode — every batch independent";
  edge [style=invis];
