...
```dot class="queue" title="A batch of three pull requests is tested together, ahead of the rest of the queue"
digraph {
  ...
  PR3 -> CI [class="muted", style=dashed, dir=both, arrowhead=none, arrowtail=normal];
}
```
...
```dot class="queue" title="batch_size: 2 — pull requests that touch the same area are grouped together"
digraph {
  edge [style=invis];
...
### Resolving Conflicts With `.gitattributes`

With `fast-forward` or `merge-batch`, the commit Mergify builds for the batch
is the one that lands, so Mergify merges the pull requests into it the way
your repository says files should be merged: it follows the `merge=`
attributes in the `.gitattributes` of your **base branch**. A pull request
that edits `.gitattributes` has no effect on its own batch; the file that
counts is the one already on the branch it merges into.

This lets a conflict on a file nobody really contends over, like a changelog
or a generated file, resolve itself instead of removing a pull request from
the batch:

```text
CHANGELOG.md        merge=union
src/generated/**    merge=prefer-theirs
```

Mergify understands these values:

| Attribute | What happens when two pull requests change the file |
|---|---|
| `merge=union` | Both sides' lines are kept. Where they conflict, both versions are included, one after the other. |
| `merge=prefer-ours` | A normal merge. Lines that conflict are taken from the batch built so far; every other change from both sides is kept. |
| `merge=prefer-theirs` | A normal merge. Lines that conflict are taken from the pull request being added; every other change from both sides is kept. |
| `merge=ours` | The whole file is kept as it is in the batch built so far. The added pull request's changes to that file are discarded, including lines that did not conflict. |
| `merge=theirs` | The whole file is taken from the pull request being added. Changes to that file from pull requests already in the batch, or merged into the base branch after the pull request was created, are discarded. |

Any other driver name is ignored and the file merges as usual.

:::caution[`ours` is not your branch]
  On your laptop, `ours` is the branch you are on. In a batch, Mergify adds
  the pull requests one at a time, so `ours` is **the batch built so far** and
  `theirs` is **the pull request being added**. When two pull requests in a
  batch change a file marked `merge=ours`, the one that entered the queue
  first wins and the other one's change to that file is dropped, without a
  conflict or a marker.

  The batch built so far starts from your base branch. So for the first pull
  request of a batch, `merge=theirs` also drops changes to that file that
  landed on the base branch after the pull request was created.

  If you want a conflict decided in one direction without losing anyone's
  other changes, use `prefer-ours` or `prefer-theirs` instead.
:::

The attributes work in both directions. A path marked `binary` or `-merge`
tells Git never to merge it, so two pull requests that edit different lines
of such a file conflict in the batch even though a plain merge would have
combined them.

Two limits to know about:

- A file deleted by one pull request and modified by another always
  conflicts. Git decides that case before any `merge=` attribute is
  consulted.
- With any other `merge_method`, each pull request is merged on its own by
  GitHub, so `merge=` attributes do not apply to the queue.
...
## Re-running Only the Previously Failed Jobs

The same idea works one level up, at the CI job. When Mergify
[splits a failed batch](#handling-batch-failure-or-timeout), a whole CI job that
passed on the parent batch passes on the smaller split too, so it does not need
to run again. Instead of re-running every job on each split, you can run only the
jobs that failed on the parent.

A test is filtered inside one process you control, so the previous recipe passes
a list of test names to your runner. A job is a whole CI unit you cannot filter
from the outside, so the mirror is self-skipping: each job checks at startup
whether it already passed on the parent batch and exits early if it did.
...
This recipe reads the same `mergify ci queue-info` output as the previous one
(see [Reading the Metadata](#reading-the-metadata)). It uses two things about the
parent batch, the last entry of `previous_failed_batches`:

- `batch_pr_head_sha`: the head commit of the parent batch's pull request. Your
  CI ran on that commit, so its check runs are the source of truth for which jobs
  passed there.

- The job's own check run on that commit: a job skips itself only when its check
  run concluded `success`. Anything else, a failure, a job that never ran, or a
  missing SHA, runs the full job.

### GitHub Actions Example: Self-skipping Jobs

This job reads the parent batch's head SHA from the metadata, looks up its own
check run on that commit, and skips its real steps when it already passed. It
falls back to running in full whenever any piece is missing, so it is safe to
use as your only workflow.
...
jobs:
  build:
    ...
    permissions:
      ...
      checks: read # read the parent batch's check runs
    steps:
      - uses: actions/checkout@v4
      ...
      - name: Skip this job if it already passed on the parent batch
        ...
        env:
          ...
          # The check-run name GitHub gives this job, matched verbatim below.
          # It is the job's `name:` (or its id when unset). See the note on
          # matrix jobs.
          JOB: build
        run: |
          # Anything we can't resolve falls back to a full run.
          fallback() { echo "$1, running the full job."; exit 0; }
          ...
          checks=$(gh api "repos/$REPO/commits/$sha/check-runs?filter=all" --paginate \
            ...
            || fallback "Could not read the parent batch check runs"

          # Skip only when this job ran there and every run of it concluded
          # success, so any failed attempt still forces a full run.
          if [ -n "$checks" ] && ! grep -qvx success <<< "$checks"; then
            echo "Already passed on the parent batch ($sha), skipping this job."
            echo "skip=true" >> "$GITHUB_OUTPUT"
          fi

      - name: Build
        if: steps.already-passed.outputs.skip != 'true'
        run: make build
```

:::note
  The optimization is silent when it does not engage: a `JOB` name that does not
  match the check run, or a missing `checks: read` permission, just runs the full
  job, with no error. `JOB` must match the check-run name GitHub assigns exactly.
  Where the job has no custom `name:` and no matrix, `${{ github.job }}` is that
  name. The one positive sign it engaged is the `Already passed on the parent
  batch` line in the job log.
:::

:::note
  Skipping happens inside the job, so the runner still starts before it exits
  early. This saves the job's work, the build or test run, not the few seconds of
  scheduling a runner. Where even starting a job is expensive, gate it with a
  lightweight planning job in `needs:` instead.
:::

:::caution
  This example targets a single job through a fixed `JOB` value, so it does not
  cover matrix jobs. Each matrix leg gets its own check-run name, the job name
  plus the matrix values in parentheses, e.g. `build (ubuntu-latest, 3.12)`. A
  static `JOB` matches at most one leg, so every other leg silently runs in full.
  To skip matrix legs, build `JOB` from the `matrix` context so each leg matches
  its own check run, e.g. `JOB: build (${{ matrix.os }}, ${{ matrix.python }})`.
:::

:::caution
  This optimization assumes a job that passed on the parent batch stays green on
  the smaller split. That holds when your jobs are independent. If a job's result
  depends on changes from another pull request in the batch, keep running it every
  time so Mergify always isolates the right culprit.
:::
...
- [`merge_queue.mode = serial`](/merge-queue/queue-modes#serial-mode)

- `queue_rules[*].max_checks_retries = 0`, since a
  [retry](/merge-queue/lifecycle#automatic-ci-retries) recreates the batch PR to
  get a fresh CI run
