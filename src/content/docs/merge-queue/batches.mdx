...
To achieve this, set `merge_method: fast-forward` on your queue rule. When
using draft PRs (i.e., `batch_size > 1`, `max_parallel_checks > 1`, or
two-step CI), Mergify creates the draft PR as usual, runs CI on it, and then
fast-forwards the base branch to the draft PR's head commit instead of
merging the original PRs individually.
...
### Migrating from `queue_branch_merge_method`

:::caution
  `queue_branch_merge_method` is **deprecated**. Use
  `merge_method: fast-forward` instead, which now works with batching and
  parallel checks.
:::

If you are currently using `queue_branch_merge_method: fast-forward`, replace
it with `merge_method: fast-forward`:
...
The new configuration produces the same result: the draft PR is tested by CI,
and the base branch is fast-forwarded to the exact SHA that was tested.
