...
You can also pause the merge queue programmatically: the [merge queue
API](/api/merge-queue) exposes pausing as a `PUT` and lifting it as a `DELETE`
on the same endpoint. See [API usage](/api/usage) for authentication.
