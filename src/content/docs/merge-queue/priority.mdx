...
In this configuration, `allow_checks_interruption` is set to `false` for the `urgent` priority.
This means if a pull request with the `urgent` label (which has a higher priority)
