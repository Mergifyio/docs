...
:::caution
  Don't pick a bypass mode to shape your merge conditions. `exempt` is the mode
  [GitHub-native stacked pull
  requests](#github-native-stacked-pull-requests) require, so you may have to
  pick it for reasons that have nothing to do with injection. Choosing a
  narrower mode to keep a rule in your conditions blocks the queue instead.
:::
...
reports also names a `tmp-` variant of that prefix, so exclude both when you
narrow a ruleset pattern.
