...
**Be careful**: Mergify injects the ruleset's rules into the merge queue as
conditions. The bypass mode you give Mergify only decides what GitHub lets it
*do*. What Mergify *requires* before merging is controlled separately, by
[`branch_protection_injection_mode`](/merge-queue/github-rulesets#controlling-injection)
on your queue rules. See [GitHub Rulesets
Compatibility](/merge-queue/github-rulesets) for bypass modes and known
incompatibilities.
