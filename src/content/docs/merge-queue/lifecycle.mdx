...
- **`auto`** (the default): Mergify computes the timeout from the queue's own
  ...
  a safety margin. Only first attempts count: a run that was
  [retried](#automatic-ci-retries) after a failure, or that tested a
  [batch split](/merge-queue/batches#handling-batch-failure-or-timeout), measures
  recovery work rather than a normal run, so Mergify leaves it out. The value
  adapts as your CI gets faster or slower, so you don't have to guess one. Until
  the queue has gathered enough CI history, `auto` applies no timeout.
...
:::note
  ...
  same holds for a queue whose recent runs were mostly retries or batch splits,
  since those don't count toward that history.
:::
