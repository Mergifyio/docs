...
A pull request can be added to the merge queue either manually or
automatically. Here's an overview of the process:

- **Manual addition:** A pull request can be manually added to the merge queue
   using the [`@mergifyio queue` command](/commands/queue) or by ticking the
   [**Queue this pull request** checkbox](/commands/queue#queueing-with-a-checkbox),
   which Mergify posts in a comment of its own once the pull request is ready to
   be queued.
...
[`queue_rules`](/merge-queue/rules). These conditions might include
requirements such as passing all checks, having a certain number of approved
reviews, or having specific labels.

Once a pull request meets the specified queue conditions, it is added to the
end of the merge queue. However, [priority rules](/merge-queue/priority) can be
used to alter its position in the queue.

```dot class="flow" title="A pull request's states in the merge queue, and what moves it between them"
digraph {
  node [width=1.8];

  Dequeued [class="external"];
  Queueing [class="pending"];
  Queued [class="queued"];
  Validating [class="pending"];
  Merged [class="merged"];

  Dequeued -> Queueing [label="  queue command\l  or auto_merge\l"];
  Queueing -> Queued [label="  matches\l  queue_conditions\l"];
  Queued -> Validating [label="  reaches the top\l  of the queue\l"];
  Validating -> Merged [label="  matches\l  merge_conditions\l", class="merged"];
  Queued -> Dequeued [label="  unmatches queue_conditions\l  or dequeue command\l", class="failed"];
  Validating -> Dequeued [label="  fails merge_conditions\l  or dequeue command\l", class="failed"];
}
```
...
3. **Validating the pull request:** When a pull request is next in line, it
   ...
   temporarily merge it with the base branch, re-triggering the CI system.
   Before the actual merge happens, it will check that the `merge_conditions`
   specified in the configuration file are satisfied. This ensures that only
   pull requests that meet the necessary conditions are merged.
...
4. **Merge success or failure:**
   ...
   - If the merge is not successful, the pull request is removed from the
     ...
     base branch or a check failure.
...
When Mergify builds a [batch](/merge-queue/batches), it merges the queued pull
requests together on a temporary branch. That composition can hit a conflict
between a pull request and another one ahead of it in the queue.

Mergify does not remove the pull request in that case. As long as the pull
request it conflicts with is still in the queue, the conflicting one is *held*:
it keeps its place, its check run reports that its checks are deferred, and the
status comment names the pull requests it is waiting on.

What happens next depends on the pull request ahead:

- **It leaves the queue without merging.** The conflict never reaches the base
  branch, so Mergify puts the held pull request back into a batch and its checks
  resume.

- **It merges.** Its changes are now part of the base branch, so the conflict is
  confirmed: the held pull request becomes conflicting with the base branch and
  is removed from the queue. Update or rebase it, then queue it again.

Two other conflicts remove the pull request from the queue immediately, without
holding it: a conflict with the base branch itself, and a conflict with a pull
request that has already left the queue.

:::note
  A deferred pull request is still queued, so it holds its position: it keeps
  the [`queued_label`](#queue-labels) and is not given the `dequeued_label`. The
  `checking_label`, if you set one, comes off until the pull request is put back
  into a batch. It is waiting for its turn, so there is nothing to requeue.
:::
...
- **`auto`** (the default): Mergify computes the timeout from the queue's own
  recent CI runtime, using the 95th-percentile duration of successful runs with
  a safety margin. The runtime is measured per queue *and* per base branch, over
  the last seven days, with recent runs weighted more heavily. Two branches
  served by the same queue can end up with different timeouts. Only first
  attempts feed that calculation: a run that was
  [retried](#automatic-ci-retries) after a failure, or that tested a batch from
  a [bisection](/merge-queue/batches#handling-batch-failure-or-timeout),
  measures recovery work rather than a normal run, so Mergify leaves it out of
  the measurement. The timeout it produces still applies to every attempt,
  retries and bisections included. The value adapts as your CI gets faster or
  slower, so you don't have to guess one. Until the queue has gathered enough CI
  history on the branch (around twenty qualifying runs in that seven-day
  window), `auto` applies no timeout.
...
:::note
  ...
  explicit duration if you want a guaranteed timeout on a brand-new queue. The
  same holds for a queue that merges only a handful of pull requests a week on a
  given branch, and for one whose recent runs were mostly retries or
  bisections, since those don't count toward that history.
:::

When a pull request is dequeued because its checks timed out, Mergify reports it
under the [`checks-timeout`](/configuration/data-types#queue-dequeue-reason)
dequeue reason, states how long it waited, and lists the checks that were still
running alongside those that never reported at all. A check that never reports
can never satisfy its merge condition, so that second list usually means a check
name in your `merge_conditions` doesn't match what your CI publishes.

When the timeout came from `auto`, the report says so ("the checks did not pass
within the automatic checks timeout of 45 minutes, derived from this queue's
recent CI runtime") and points at `checks_timeout` so you can set your own value
or `null` to disable it. A configured duration is reported on its own, with no
mention of where it came from. For batches, see [Handling Batch Failure or
Timeout](/merge-queue/batches#handling-batch-failure-or-timeout).

## Automatic CI Retries

Flaky CI can remove a pull request that would otherwise merge cleanly. The
`max_checks_retries` option in your [`queue_rules`](/merge-queue/rules) tells
Mergify how many times to retry failed CI checks before removing the pull
request from the queue. On each retry, Mergify recreates the draft pull request
to trigger a fresh CI run.
...
`max_checks_retries` defaults to `0`, which disables retries and keeps the
normal failure handling described below. When you set it greater than `0`,
Mergify always creates a draft pull request and disables in-place checks, since
each retry needs a fresh draft to run against. The queue status report shows the
current attempt number, for example "attempt 2/3".
...
An interrupted check is the exception to the steps above. When a check blocking
the merge conditions was cancelled before reaching any conclusion (a reclaimed
runner, a cancelled workflow run), Mergify removes the pull request under the
[`checks-interrupted`](/configuration/data-types#queue-dequeue-reason) dequeue
reason rather than `checks-failed`, and names the checks that never reached a verdict.
Nothing about the pull request needs fixing: requeue it with `@mergifyio queue`,
without updating it, and the check runs again. [Automatic CI
retries](#automatic-ci-retries) come first, so a queue with `max_checks_retries`
set only reports the interruption once its retries are spent.

A check that reported a failure of its own is a failure rather than an
interruption, and so is a check that timed out on its own (a `timed_out`
conclusion, matched by
[`check-timed-out`](/configuration/conditions#attributes-list)): it used up its
own time rather than being stopped from outside. When a check fails and another
is interrupted at the same time, the failure wins, and the pull request is
removed under `checks-failed`.
...
using the [`@mergifyio queue` command](/commands/queue), which will reset its
...
branch. Therefore, if a new commit is pushed manually to the base branch,
Mergify resets the process and starts over with the updated base branch.
...
- Its name is the
  [`queue_branch_prefix`](/configuration/file-format/#queue-rules), followed by
  exactly 10 hexadecimal characters (`0`–`9`, `a`–`f`).
...
Only that shape is matched. A queue branch is pushed straight to its final
name, and a branch whose name merely starts with your prefix is left alone.
...
Every pull request in a repository with the merge queue enabled shows a single
"Mergify Merge Queue" check run. This check consolidates the pull request's
queue status, whether it is queued, waiting for conditions, or has been
dequeued, into one consistent check. You can read it from the "checks" tab
alongside the other Mergify statuses.

## Queue Labels

Mergify can label a pull request according to where it stands in the queue.
Three options under `merge_queue` control this, and each takes the name of the
label to apply:

- **`queued_label`** (`queued` by default): applied while the pull request sits
  in the queue, from the moment it is queued until it leaves.

- **`checking_label`** (no label by default): applied while the queue is
  actually checking the pull request, that is, while it is embarked in a batch.
  It stays on a pull request parked in a
  [bisection](/merge-queue/batches#handling-batch-failure-or-timeout), whose own
  CI has not started yet. It comes off if the pull request goes back to waiting
  in the queue, and again when it leaves the queue.

- **`dequeued_label`** (`dequeued` by default): applied once the pull request has
...
Where `queued_label` marks everything in the queue, `checking_label` narrows that
down to the pull requests the queue is working on right now, so you can tell at a
glance which ones are consuming CI capacity.

## Status Comments

Mergify posts status comments on each pull request in the queue to report its
progress. The `status_comments` option in your `merge_queue` settings controls
how much Mergify posts:

- **`all`** (the default): comments for every queue event, including entering
  the queue, CI progress, and final outcomes.

- **`outcomes`**: comments only for final outcomes, such as a merge or a dequeue
