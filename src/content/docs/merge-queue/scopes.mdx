...
pull requests. See the [GitHub Actions](/integrations/gha#job-selection) and
[Buildkite](/integrations/buildkite#job-selection) integrations.
...
```dot class="queue" title="Scope-aware batch selection: the three pull requests that share frontend and api form the batch; the tooling change waits"
digraph {
  edge [arrowhead=none];
  ...
  subgraph cluster_preferred {
    ...
    PR1 [label="PR #101\nScopes: frontend, api", class="queued"];
    PR2 [label="PR #214\nScopes: frontend", class="queued"];
    PR3 [label="PR #305\nScopes: api, docs", class="queued"];
  }

  PR4 [label="PR #412\nScope: tooling", class="muted"];

  frontend [shape=oval, class="config"];
  api [shape=oval, class="config"];
  docs [shape=oval, class="config"];
  tooling [shape=oval, class="config"];
...
- **Manual upload:** Use the [`gha-mergify-ci`](https://github.com/Mergifyio/gha-mergify-ci)
  ...
  computed by your own tooling. There is a build tool guide for [Nx](/merge-queue/scopes/nx),
  [Bazel](/merge-queue/scopes/bazel), [Pants](/merge-queue/scopes/pants) and
  [Turborepo](/merge-queue/scopes/turborepo), plus one for
  [any other build tool](/merge-queue/scopes/others).

### Limits on an upload

An upload sent through the [REST API](/api/pull-requests) or the [Mergify CLI](/cli/usage) has to
stay within these limits:

- A scope name is at most 1024 characters.
- A scope name cannot contain a NUL byte or an unpaired UTF-16 surrogate.
- An upload carries at most 10,000 scopes.

An upload that breaks any of them is refused in full with a `422`, and nothing is stored: the pull
request keeps whatever was uploaded for it last, which is the stale result of an earlier run rather
than no result at all. If your detection tooling can emit a generated name (a path from a build
graph, for example), cap its length on your side, so an over-long name fails the job that produced
it instead of leaving the queue to batch on the previous run's scopes.
...
- `scopes.capacities`: optional map of scope name to the number of speculative checks that scope may
  ...
  concurrency per scope](/merge-queue/queue-modes#limiting-concurrency-per-scope). A key holding `*`,
  `?`, `[` or `{` is a glob over scope names, with the syntax of the [glob
  patterns](/configuration/data-types#globs) a `files` source uses; any other key names one scope
  exactly. With a `manual` source an exact key may be any name the scopes API accepts, such as a
  Bazel package label like `//lib/core` or a scoped package name like `@acme/web`. Under any other
  source it must be at least two characters and match `^[A-Za-z0-9_-]+$`, the same constraint the
  `files` source puts on the scope names you declare under it.

- `scopes.default_capacity`: optional capacity applied to every scope that no `capacities` key
  matches, so scope sets that change from one pull request to the next stay bounded without you
