...
You can send the flag through the [REST API](/api/pull-requests) as well. This
endpoint needs an application key with the
[`ci` scope](/api/usage#application-key-scopes); an `admin` key is rejected:
