...
- `scopes.capacities`: optional map of scope name to the number of speculative checks that scope may
  ...
  concurrency per scope](/merge-queue/queue-modes#limiting-concurrency-per-scope). Its keys must be
  at least two characters and match `^[A-Za-z0-9_-]+$`, the same constraint the `files` source puts
  on the scope names you declare under it. Uploaded scope names carry no such constraint, so a scope
  name that contains anything else, such as a Bazel package label like `//lib/core` or a scoped
  package name like `@acme/web`, cannot have a `capacities` entry. Use `scopes.default_capacity` to
  bound those.
