...
Nx into the `mergify-ci.scopes` meta-data, and a third uploads them. The
...
<ScopesDetection
  command={String.raw`# A failed nx piped into jq still exits 0 and writes a well-formed, empty
