...
`$HEAD` revisions each pipeline resolves for itself:
...
- Whichever grain you pick, the label can be given a capacity of its own:
  [`scopes.capacities`](/merge-queue/queue-modes#limiting-concurrency-per-scope) takes any name the
  scopes API accepts when the source is `manual`, so `//lib/core: 2` caps that package. A key
  holding `*`, `?`, `[` or `{` is [a glob over scope
  names](/merge-queue/queue-modes#naming-scopes-with-patterns) instead, so `//lib/**` gives every
  package under `//lib` its own budget. To bound the rest,
...
Every pipeline here uses the released JAR regardless, for one reason: they check out two revisions of
...
Each pipeline downloads a JAR over the network and runs it with the same privileges as the rest
