...
- Whichever grain you pick, a Bazel label is not a name
  [`scopes.capacities`](/merge-queue/queue-modes#limiting-concurrency-per-scope) accepts: its keys
  must match `^[A-Za-z0-9_-]+$`, and `//lib/core` does not. Uploaded scope names are unconstrained,
  so the label works everywhere else. To bound how many speculative checks these scopes run at once,
  use [`scopes.default_capacity`](/merge-queue/queue-modes#capping-scopes-you-did-not-list), which
  applies to every scope the queue observes rather than to names you list.
