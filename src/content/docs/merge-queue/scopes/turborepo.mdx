...
Whichever recipe you use, the scopes it uploads are the package names `turbo` reports under
`.packages`, and a scoped name such as `@acme/web` can be given a capacity of its own:
[`scopes.capacities`](/merge-queue/queue-modes#limiting-concurrency-per-scope) takes any name the
scopes API accepts when the source is `manual`. Quote it in YAML, since a key starting with `@` is
not a plain scalar. A key holding `*`, `?`, `[` or `{` is [a glob over scope
names](/merge-queue/queue-modes#naming-scopes-with-patterns) instead, so `'@acme/*'` gives every
`@acme/` package its own budget. To bound the rest,
