...
`.packages`, and a scoped name such as `@acme/web` is not a name
[`scopes.capacities`](/merge-queue/queue-modes#limiting-concurrency-per-scope) accepts: its keys must
match `^[A-Za-z0-9_-]+$`. Uploaded scope names carry no such constraint, so the package name works
everywhere else in the queue. To bound how many speculative checks these scopes run at once, use
