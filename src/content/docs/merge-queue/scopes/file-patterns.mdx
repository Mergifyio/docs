...
touch. This is optional and independent of merge queue batching. To wire it up, follow the Monorepo
CI section of the [GitHub Actions integration](/integrations/gha#monorepo-ci). It walks through:
...
That section already includes complete workflow samples, so you do not need to duplicate the config
here. Just reuse the same scopes you declared above. The
[Buildkite integration](/integrations/buildkite#monorepo-ci) walks through the first two steps for
Buildkite pipelines, using a dynamic pipeline instead of job conditions.
...
Gate the job on the scope, the same way you gate any other. `detect-scopes` below is the job from
the [GitHub Actions integration](/integrations/gha#monorepo-ci), with `merge-queue` added to its
`outputs`:
