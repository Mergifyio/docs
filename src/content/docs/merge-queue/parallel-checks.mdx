...
value. It creates batch PRs that speculatively combine changes and runs your CI
on each in parallel. For example, with PR #1–#5 and `max_parallel_checks: 3`,
Mergify creates:

- Batch #1: PR #1
- Batch #2: PR #1 + PR #2
- Batch #3: PR #1 + PR #2 + PR #3
...
strict digraph "Parallel Checks" {
  ...
  subgraph cluster_mergequeue {
    ...
    subgraph cluster_specmerge {
      ...
      // Add a bit of padding around labels inside batch nodes
      node [];
      style="rounded";
      color="#000";
      fontcolor="#000";
...
If a batch fails, Mergify removes the culprit PR from the queue and continues
with the rest.
...
:::note
  Mergify merges the original PRs, not the temporary batch PRs. If you prefer
  merging the batch PRs instead, see [Merging the Batch
  PRs](/merge-queue/batches#merging-the-batch-prs).
:::
...
```text
Batch #1 (PR #1):           ❌ Failed (flaky test)
Batch #2 (PR #1 + PR #2):   ✅ Passed
Batch #3 (PR #1 + #2 + #3): ✅ Passed
```
...
**With `skip_intermediate_results: true`**: Mergify sees that Batch #2
(which contains PR #1's code) passed. This proves PR #1's failure was
flaky, not a real bug. All 3 PRs merge automatically.

### How It Works

When enabled, Mergify doesn't require every intermediate batch to pass.
Instead, it looks for **any** passing batch that contains the PR's changes:

1. Batch #1 fails (contains PR #1)

2. Batch #2 passes (contains PR #1 + PR #2)

3. Since Batch #2 passed and includes PR #1's code, the failure in Batch #1
   is treated as transient

4. Both PR #1 and PR #2 merge

This is safe because if PR #1 had a **real** bug, Batch #2 would also fail
