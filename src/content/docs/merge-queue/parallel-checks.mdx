...
temporary draft PRs) only when ALL of the following are true:
...
In‑place checks are implicitly enabled by the combination above. 
If any of these conditions is not met, Mergify will only use speculative draft 
PRs for validation.
