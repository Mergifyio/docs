...
- **[GitHub-native stacked pull
  ...
  [gh-stack](/stacks/compare/gh-stack) or by a plain `mergify stack push`.
  Every member can be queued, and members land one at a time from the bottom
  up.

- **[Mergify Stacks](#mergify-stacks)**, created with
  `mergify stack push --no-github-native`. The queue treats the whole chain as
  a unit: it propagates the queue command up the stack, keeps stacked PRs
  together when batching, and dequeues the PRs above any PR you take out of
  the queue.
...
CLI, or with `mergify stack push`, which registers the stack with GitHub's
stacking API unless you pass `--no-github-native`. Each member is a separate
branch targeting the branch below it.
...
branch to its own pull request. The queue treats such a stack as one unit.
...
This is exactly what [`mergify stack push`](/stacks/creating) with
[`--no-github-native`](/stacks/setup#configuration) produces. PRs chained only
by branch refs (for example, GitFlow promotion chains like `dev` → `staging` →
`prod`) are **not** treated as a stack. Without the `Depends-On:` marker, the
queue keeps each PR's literal base ref and queues them independently.

`mergify stack push` registers the stack with GitHub's stacking API unless you
pass `--no-github-native`. A registered stack carries no `Depends-On:` marker
either, because that API holds the ordering instead, so those PRs are
