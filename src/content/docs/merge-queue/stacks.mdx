...
:::note
  Merging GitHub-native stacks through the Merge Queue is a progressive
  rollout, not on by default for every organization. [Contact
  support](/support) to have it turned on for yours; until then, the queue
  refuses to merge any member of a GitHub-native stack and asks you to merge
  the stack from the GitHub UI instead.
:::
