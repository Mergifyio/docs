...
4. **Get [Direct Merge](/merge-queue/direct-merge)** once a pull request carries scopes, whichever
   way they were attached: when the base branch has moved only in scopes that pull request does not
   touch, it merges immediately with no queue CI run. Other conditions apply, listed on that page.

Need scope-aware CI without the merge queue? The [GitHub
Actions](/integrations/gha#job-selection) and [Buildkite](/integrations/buildkite#job-selection)
integrations show how to skip unaffected CI jobs with the same scopes.
...
Bazel, or Turborepo first. When you also want scope-aware CI, the [GitHub Actions
integration](/integrations/gha#job-selection) ships a ready-made
...
- [GitHub Actions](/integrations/gha#job-selection) and
  [Buildkite](/integrations/buildkite#job-selection) integrations: reuse the same scopes to skip
  unaffected CI jobs on pull requests.
