...
| Stack visualization | Stack comment + browser-extension panel, or `--github-native` | Native GitHub UI stack map |
...
[`mergify stack push --github-native`](/stacks/setup#configuration) registers
the stack with GitHub's own stacking API, so GitHub displays it as a stack, but
that flag is experimental and is skipped where the API is not available.
