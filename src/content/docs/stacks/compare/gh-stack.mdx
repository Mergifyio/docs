...
<GitGraph
  nodes={[
    { id: "m1", label: "M", role: "external" },
    { id: "m2", label: "", role: "external" },
    { id: "a1", label: "A", role: "queued" },
    { id: "a2", label: "A'", role: "queued" },
    { id: "a3", label: "", role: "queued" },
    { id: "b1", label: "B", role: "config" },
    { id: "b2", label: "", role: "config" },
    { id: "c1", label: "C", role: "pending" },
    { id: "c2", label: "C'", role: "pending" },
  ]}
  edges={[
...
|---|---|---|
| Create a stack | `mergify stack new <name>` + commits | `gh stack init` + `gh stack add` per branch |
...
| Stack visualization | Stack comment, browser-extension panel, GitHub's stack map | Native GitHub UI stack map |
...
| Exit codes | [Structured (8 specific codes)](/cli/usage#exit-codes) | Structured (8 specific codes) |
...
queue, which is more limited. Mergify's Merge Queue also lands
[GitHub-native stacks](/merge-queue/stacks#github-native-stacked-pull-requests),
keeping their members together and in order, so you can use GitHub's stacking
UI and Mergify's queue at the same time.

**Collaboration support.** `mergify stack checkout` lets a teammate pick up your
stack locally and push changes. It takes the stack's remote branch name or the
URL of any pull request in it. With gh-stack's force-push model, coordinating on
the same stack is harder.
...
stack and its revision history as an in-PR panel. Neither of those is a
built-in GitHub UI element.
[`mergify stack push`](/stacks/setup#configuration) registers the stack with
GitHub's own stacking API by default, so GitHub displays it as a stack. That
registration is experimental and is skipped where the API is not available.
...
|---|---|
| `gh stack init` | `mergify stack new <name>` |
