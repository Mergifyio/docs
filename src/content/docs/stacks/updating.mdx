...
`--dry-run` (`-n`) to print the plan without touching your branch. The
[`stack` CLI reference](/cli/stack) has the full argument list for each one.

`mergify stack push` rebases your branch on its base branch before pushing, but
only while no PR in the stack carries an approval. Once a reviewer has
approved, it pushes without rebasing, because rebasing would dismiss that
review. Pass `--force-rebase` to rebase anyway and accept the dismissals, or
`--skip-rebase` (`-R`) to never rebase.
...
```bash
mergify stack reorder d4e5f6a a1b2c3d b7c8d9e
```
...
```bash
mergify stack move b7c8d9e first
...
```bash
mergify stack reorder d4e5f6a a1b2c3d b7c8d9e --dry-run
```
