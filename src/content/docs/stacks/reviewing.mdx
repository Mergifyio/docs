...
merges first, and the next one is retargeted onto `main`. GitHub does that
itself for a stack registered with its stacking API. For a stack pushed with
[`--no-github-native`](/stacks/setup#configuration), it happens when the merged
head branch is deleted, which is why [Setup](/stacks/setup) asks you to turn
automatic head-branch deletion on. The author brings the rest of the
stack in line by running
[`mergify stack sync`](/stacks/updating#after-a-pr-merges) and pushing again.
You review and merge each PR in turn, working up the chain.
