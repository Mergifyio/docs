...
merges first. GitHub then retargets the next PR onto `main`, but only if the
repository deletes head branches on merge, which is why [Setup](/stacks/setup)
asks you to turn that on. The author brings the rest of the stack in line by
running [`mergify stack sync`](/stacks/updating#after-a-pr-merges) and pushing
again. You review and merge each PR in turn, working up the chain.
