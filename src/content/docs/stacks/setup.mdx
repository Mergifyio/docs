...
:::note
  ...
  `Depends-On:` chain. Stacks skips the registration silently where that API
  isn't available, and the push goes through as usual. Pass
  `--no-github-native`, or set the Git config to `false`, to push without it.
:::
