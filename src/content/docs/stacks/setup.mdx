...
Stacks needs access to create PRs. Set the `MERGIFY_TOKEN` or `GITHUB_TOKEN`
environment variable, or install the [GitHub CLI](https://cli.github.com/) and
run `gh auth login`. The CLI tries them in that order. See
[Authentication](/cli/usage#authentication) for where each token comes from.
