...
1. **Rebases** your branch on the latest `main`, unless a PR in the stack is
   already approved and the rebase would dismiss the review (see
   [Updating Stacks](/stacks/updating))
...
5. **Registers the stack** with GitHub's stacking API, so GitHub shows it as a
   stack of its own and holds the ordering. Pass
   [`--no-github-native`](/stacks/setup#configuration) to push without this and
   get a [`Depends-On:` chain](/merge-queue/stacks#mergify-stacks) instead,
   which the [Merge Queue](/merge-queue/stacks) moves as a unit just the same
...
:::tip
  Use `mergify stack open` to quickly open a PR in your browser. Name a commit
  to open its PR; run it without one from a terminal and you get an interactive
  picker with `HEAD` preselected.
:::
