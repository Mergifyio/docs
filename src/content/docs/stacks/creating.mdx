...
When your base branch is `origin/main`, this is equivalent to
`git checkout -b feat/my-feature --track origin/main`. Without `--base`, the
base branch comes from the current branch's tracking information, falling back
to `origin`'s default branch. You can also create branches however you normally
do. Stacks doesn't require its own branch creation command.
...
The rebase in step 1 is skipped once a PR in the stack is approved, since
rebasing would dismiss the approval. Pass `--force-rebase` to rebase anyway, or
