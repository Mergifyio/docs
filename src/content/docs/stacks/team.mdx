...
pair, or continue the work), use `checkout` with the stack name (the name of
the local branch they pushed the stack from) and their GitHub login:

```bash
mergify stack checkout feat/their-feature --author their-login
```

Without `--author`, `checkout` looks for a stack of that name authored by you,
which is also how you pick up one of your own stacks from another machine.

`checkout` reconstructs the stack locally by fetching the remote branches and
