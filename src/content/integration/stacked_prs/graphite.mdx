...
To configure Mergify for your repository, modify the [Mergify configuration file](/configuration/file-format) on the `main` branch. Set a condition on the `base` attribute to be equal to the name of your trunk branch (`main` in our sample stack). This ensures that Mergify only merges pull requests that are at the bottom of their stack.
...
If you make changes to your Mergify configuration file, make sure your changes are on the `main` branch and that your stack is up to date. Execute the following commands to ensure this:
...
Now that everything is set up, you can queue your pull request. Wait a few seconds and observe Mergify automatically merging pull request A into `main`. GitHub will then delete the branch A and set `main` as the base branch for pull request B.
