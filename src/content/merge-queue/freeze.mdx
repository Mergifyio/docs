...
queue, you need to navigate to the queue page in your repository and look for
the option labeled "Freeze Queue". Clicking on this option will effectively
pause all merges for that specific queue.
...
click on the "Unfreeze Queue" option to resume the merge operations. As soon as
a queue is unfrozen, the merge operations continue from where they left off,
assuming all checks and conditions are still met.
