...
Two things that follow. Always name the org in the URL — the dashboard scopes
pages as `/orgs/<org>[/repos/<repo>]/…`, and a URL with no org in it inherits
whatever org the session last used, which is the actual way this goes wrong. And
even in the sandbox, crop to the content pane so the sidebar's org/user header
stays out of frame. A dashboard dev build rendered from seeded mock data with no session at
